...
class Leak_Protection extends Functions {
	...
	public function filter_oembed_post_id( $post_id, $url ) {
		...
		if ( $post_id <= 0 || 'excerpt' === dtac_give_get_leak_mode() ) {
			return $post_id;
		}
	...
	private function restricted_message(): string {
		...
		return esc_html__( 'This content is restricted until a donation is made.', 'dtac-give' );
	}
	...
	private function restricted_excerpt( int $post_id ): string {
		...
		if ( '' === trim( (string) $excerpt ) ) {
			$excerpt = wp_trim_words( wp_strip_all_tags( (string) $post->post_content ), 40, '&hellip;' );
		}

		return wp_kses_post( (string) $excerpt );
	}
}
